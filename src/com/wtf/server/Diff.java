...
/*
 * $Log: Diff.java,v $ Revision 1.7 2009/01/19 03:05:26 stuart Fix StackOverflow
 * bug with heuristic on reported by Jimmy Han.
 * 
...
public class Diff {
  ...
  private int diag(int xoff, int xlim, int yoff, int ylim) {
    ...
    for (int c = 1;; ++c) {
       ...
       * progress compared with the edit distance. If we have any such, find the
       * one that has made the most progress and return it as if it had
       * succeeded.
       * 
  ...
  private void compareseq(int xoff, int xlim, int yoff, int ylim) {
    ...
    if (xoff == xlim)
      ...
      if (c == 1) {
         ...
         * subsequences is empty, and that case was handled above without
         * calling `diag'. Let's verify that this is true.
         */
  ...
  public change diff(final ScriptBuilder bld) {
    ...
    /*
     * Now do the main comparison algorithm, considering just the undiscarded
     * lines.
     */
  ...
  class file_data {
    ...
    void clear() {
      /*
       * Allocate a flag for each line of each file, saying whether that line is
       * an insertion or deletion. Allocate an extra element, always zero, at
    ...
    private byte[] discardable(final int[] counts) {
      ...
      /*
       * Multiply MANY by approximate square root of number of lines. That is
       * the threshold for provisionally discardable lines.
       */
      while ((tem = tem >> 2) > 0)
        many *= 2;
      ...
      for (int i = 0; i < end; i++) {
        ...
        if (discards[i] == 2)
          ...
          if (provisional * 4 > length) {
            ...
            /*
             * MINIMUM is approximate square root of LENGTH/4. A subrun of two
             * or more provisionals can stand when LENGTH is at least 16. A
             * subrun of 4 or more can stand when LENGTH >= 64.
             */
            while ((tem = tem >> 2) > 0)
              minimum *= 2;
            minimum++;
             ...
             * nonprovisionals in a row or until the first nonprovisional at
             * least 8 lines in. Until that point, cancel any provisionals.
             */
            for (j = 0, consec = 0; j < length; j++) {
    ...
    void shift_boundaries(file_data f) {
      ...
      for (;;) {
        ...
        /*
         * Scan forwards to find beginning of another run of changes. Also keep
         * track of the corresponding point in the other file.
         */

        while (i < i_end && !changed[1 + i]) {
          while (other_changed[1 + j++])
            /*
             * Non-corresponding lines in the other file will count as the
             * preceding batch of changes.
             */
            other_preceding = j;
          i++;
        }
        ...
        for (;;) {
          ...
          /*
           * If the first changed line matches the following unchanged one, and
           * this run does not follow right after a previous run, and there are
           * no lines deleted from the other file here, then classify the first
           * changed line as unchanged and the following line as changed in its
           * place.
           */
          ...
          if (end != i_end
            ...
            ++i;
            /*
             * Since one line-that-matches is now before this run instead of
             * after, we must advance in the other file to keep in synch.
             */
            ++j;
          } else
            break;
